...
void colourLoop() {
  int _input = analogRead(COLOURPIN);
  _input = kalmanColour.updateEstimate(_input);
  if (_input > SILVER - 10 && _input < SILVER + 10){
      silverFlag = true;
      whiteFlag = false;
      blackFlag = false;
  }else if (_input > NORMAL - 10 && _input < NORMAL + 10){
      silverFlag = false;
      whiteFlag = true;
      blackFlag = false;
  }else if (_input > HAZARD - 10 && _input < HAZARD + 10){
      silverFlag = false;
      whiteFlag = false;
      blackFlag = true;
  }
